"""
    ...
    plate dims, sample dims, and underlying latent dims.
...
class WrappedDist:
    """
    Thin wrapper over a Torch Distribution providing NamedTensor sampling and eval.
    
    arg:
        - **dist (torch.distributions.distribution.Distribution):**  
        - **args**: list of dimension names
    """
...
class SampleLogProbK():
    ...
    We enforce the following order on `names` list: [*plates, _K, *pos]
    where "_K" is a placeholder for the current sampled variable.
    
    arg:
        - **K (int):** number of copies of latents / simultaneous samples drawn
        - **protected_dims (int):** number of placeholder dimensions to account for
    """
...
class LogProbK():
    ...
    Must follow a call to `SampleLogProbK` to obtain `in_dict['samples']`
    We enforce the following order on `names` list: [*arg, *plates, *pos]
    ...
    def __init__(self, dim_names): #plate_names, protected_dims):
        ...
        self.plate_names = dim_names[:-(self.protected_dims+1)]
    ...
    def delete_names(self, trace_prefix, names, tensors):
        ...
        Controls dimensionality in large models with high K.
        Must put ALL tensors that used in future through this function, 
        otherwise dims won't align
        
        arg:
            - **trace_prefix (PrefixTrace):** trace that generated these tensors; supplies the appropriate index prefix string.
            - **names (list):** dims to delete from `tensors`
            - **tensors (list):** list of NamedTensors
        """
        names = [k_dim_name(concat_prefix(trace_prefix.prefix, name)) \
                 for name in names]

        for name in names:
            # `remove` intentionally errors if name isn't present.
            self.arg_names.remove(name)

        # squeeze errors if name isn't present, or if dimension shape > 1
...
class PrefixTrace():
    ...
    Examples:
        - `trace( {"data":data}, SampleLogProbK(K=draws, protected_dims=nProtected) )`
        
    arg:
        - **prefix (str):** string to prepend
        - **trace (Trace):** object representing a model, and holding inputs / outputs / function to call
    """
...
# Look in torch.distributions, wrap each listed dist in our WrappedDist
# then give the Trace object a function with that name
dist_names = [
    "Bernoulli",
    "Beta",
    "Binomial",
...
class Trace:
    """
    Object to store previous results, next results, and the relevant function.
    calling trace["a"] returns a new object, with refs to the same dicts, 
    with an extended prefix

    Each dict of dicts can include:
...
def rename_placeholders(reference, placeheld) :
    """
    Rename and align dimensions in the placeheld trace to the order in the reference trace
    
    args:
        - **reference (PrefixTrace):**
        - **placeheld (PrefixTrace):**
    """
...
def sum_out_pos(tr) :
    for kind, d in tr.trace.out_dicts.items() :
        for k, T in d.items() :
            posses = [dim for dim in T.names if dim]
            posses = [dim for dim in posses \
                      if pos_name("") in dim]
            for pos in posses :
                d[k] = d[k].sum(pos)
...
def sampler(draws, nProtected):
    """
        Helper to provide the first-pass function, SampleLogProbK, with a new appropriate trace. 
        Note that `trace` instantiates a PrefixTrace which inits a Trace.
    """
...
def evaluator(wrapper, nProtected, data={}) :
    """
        Helper to provide the second-pass function, LogProbK, with a new appropriate trace. 
    """
    tr = wrapper.trace
    ...
    lpk = LogProbK(dim_names=tr.fn.dim_names)
...
if __name__ == "__main__" :
    ...
    def chain_dist(trace):
        a = trace["a"].Normal( t.ones(3), 3)
        b = trace["b"].Normal(a, 3)
        c = trace["c"].Normal(b, 3)
        (c,) = trace.delete_names(("a", "b"), (c,))
        d = trace["d"].Normal(c, 3)
    ...
    tr1 = trace({}, SampleLogProbK(K=4, protected_dims=2))
    #a = tr1["a"].Normal(t.ones(3), 3)
    val = chain_dist(tr1)
